function [gradRp]=grad_Rp_f(par,tp);
...
term1= (1/a)*nu*(-(C*Rs)/a+(b/a^2)+C)*(-(terma/termb-1));
term2= (terma/termb.*(((Emax-tp*nu)/(b*nu))-a*(Emax-tp*nu)/(b*Rp*nu)));
term3= 2*(Emax/(b*nu)-Emax*a/(b*Rp*nu))*exp((a*(Emax-tp*nu)/(b*nu))+(Emax*a/(b*nu)))/(termb^2);
term4= nu*(tp- (C*Rp - b/a)).*((terma/termb)-1)/(a^2) ;

gradRp = term1+termc.*(term2-term3) - term4;
end